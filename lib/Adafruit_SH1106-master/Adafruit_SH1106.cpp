...
uint16_t Adafruit_SH1106::getPixel(int16_t x, int16_t y) {
  ...
  if( buffer[x+ (y/8)*SH1106_LCDWIDTH] & (1 << (y&7)) !=0 ) return BLACK;
  else return WHITE;
}

void Adafruit_SH1106::invertPixel(int16_t x, int16_t y) {
  return drawPixel(x, y, INVERSE);
}

void Adafruit_SH1106::invertRect(int16_t x, int16_t y, int16_t w, int16_t h) {
  ...
  if (y + h > height()) y = height() - y - 1;

  // x is which column
  for(int16_t i=x;i<x+w;++i){ // pour chaque colonne
    int16_t j=y;
    while(j<y+h && j%8!=0){ // debut (octets non complets)
      buffer[i + (j / 8)*WIDTH] ^= (1 << (y&7));
      ++j;
    }
    int16_t octet=i + (j / 8)*WIDTH;
    while((j+8)<y+h){ // milieu, (octets complets)
      buffer[octet] ^= B11111111;
      octet+=WIDTH;
      j+=8;
    }

    while(j<y+h){ // fin (octets non complets)
      buffer[i + (j / 8)*WIDTH] ^= (1 << (y&7));
      ++j;
    }
  }
