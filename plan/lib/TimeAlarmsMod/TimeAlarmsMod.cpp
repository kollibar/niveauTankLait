...
AlarmID_t TimeAlarmsClass::triggerOnce(time_t value, OnTick_arg_t onTickHandler, pointeurArg arg){   // trigger once at the given time_t
     if( value > 0)
        return create( value, onTickHandler, arg, IS_ONESHOT, dtExplicitAlarm );
     else
        return dtINVALID_ALARM_ID; // dont't allocate if the time is greater than one day 	  
}
...
AlarmID_t TimeAlarmsClass::alarmOnce(time_t value, OnTick_arg_t onTickHandler, pointeurArg arg){   // trigger once at the given time of day
     if( value <= SECS_PER_DAY)
        return create( value, onTickHandler, arg, IS_ONESHOT, dtDailyAlarm );
     else
        return dtINVALID_ALARM_ID; // dont't allocate if the time is greater than one day 	  
}
...
AlarmID_t TimeAlarmsClass::alarmOnce(const int H,  const int M,  const int S,OnTick_arg_t onTickHandler, pointeurArg arg){   // as above with HMS arguments
   return create( AlarmHMS(H,M,S), onTickHandler, arg, IS_ONESHOT, dtDailyAlarm );
}
...
AlarmID_t TimeAlarmsClass::alarmOnce(const timeDayOfWeek_t DOW, const int H,  const int M,  const int S, OnTick_arg_t onTickHandler, pointeurArg arg){  // as above, with day of week 
   return create( (DOW-1) * SECS_PER_DAY + AlarmHMS(H,M,S), onTickHandler, arg, IS_ONESHOT, dtWeeklyAlarm );   
}
...
AlarmID_t TimeAlarmsClass::alarmRepeat(time_t value, OnTick_arg_t onTickHandler, pointeurArg arg){ // trigger daily at the given time
    if( value <= SECS_PER_DAY)
       return create( value, onTickHandler, arg, IS_REPEAT, dtDailyAlarm );
    else
       return dtINVALID_ALARM_ID; // dont't allocate if the time is greater than one day 	  
}
...
AlarmID_t TimeAlarmsClass::alarmRepeat(const int H,  const int M,  const int S, OnTick_arg_t onTickHandler, pointeurArg arg){ // as above with HMS arguments
    return create( AlarmHMS(H,M,S), onTickHandler, arg, IS_REPEAT, dtDailyAlarm );
}
...
AlarmID_t TimeAlarmsClass::alarmRepeat(const timeDayOfWeek_t DOW, const int H,  const int M,  const int S, OnTick_arg_t onTickHandler, pointeurArg arg){  // as above, with day of week 
    return create( (DOW-1) * SECS_PER_DAY + AlarmHMS(H,M,S), onTickHandler, arg, IS_REPEAT, dtWeeklyAlarm );      
}
...
AlarmID_t TimeAlarmsClass::timerOnce(time_t value, OnTick_arg_t onTickHandler, pointeurArg arg){   // trigger once after the given number of seconds 
    return create( value, onTickHandler, arg, IS_ONESHOT, dtTimer );
}
...
AlarmID_t TimeAlarmsClass::timerOnce(const int H,  const int M,  const int S, OnTick_arg_t onTickHandler, pointeurArg arg){   // As above with HMS arguments
    return create( AlarmHMS(H,M,S), onTickHandler, arg, IS_ONESHOT, dtTimer );
}
...
AlarmID_t TimeAlarmsClass::timerRepeat(time_t value, OnTick_arg_t onTickHandler, pointeurArg arg){ // trigger after the given number of seconds continuously
    return create( value, onTickHandler, arg, IS_REPEAT, dtTimer);
}
...
AlarmID_t TimeAlarmsClass::timerRepeat(const int H,  const int M,  const int S, OnTick_arg_t onTickHandler, pointeurArg arg){ // trigger after the given number of seconds continuously
    return create( AlarmHMS(H,M,S), onTickHandler, arg, IS_REPEAT, dtTimer);
}
      ...
      if(! isServicing)
      {
        ...
        for( servicedAlarmId = 0; servicedAlarmId < dtNBR_ALARMS; servicedAlarmId++)
        {
          if( Alarm[servicedAlarmId].Mode.isEnabled && (now() >= Alarm[servicedAlarmId].nextTrigger)  )
          {
            ...
            OnTick_arg_t TickArgHandler = Alarm[servicedAlarmId].onTickArgHandler;
            pointeurArg arg = Alarm[servicedAlarmId].arg;
            if(Alarm[servicedAlarmId].Mode.isOneShot)
               free(servicedAlarmId);  // free the ID if mode is OnShot		
            else   
               Alarm[servicedAlarmId].updateNextTrigger();
            if( TickHandler != NULL)  (*TickHandler)(); // call the handler
            else if( TickArgHandler != NULL) (*TickArgHandler)(arg);     // call the handler    avec un argument (type pointeurArg)
          }
        }
        isServicing = false;
      }
...
AlarmID_t TimeAlarmsClass::create( time_t value, OnTick_arg_t onTickHandler, pointeurArg arg, uint8_t isOneShot, dtAlarmPeriod_t alarmType, uint8_t isEnabled) 
    {
      if( ! (dtIsAlarm(alarmType) && now() < SECS_PER_YEAR)) // only create alarm ids if the time is at least Jan 1 1971
      {  
    	for(uint8_t id = 0; id < dtNBR_ALARMS; id++)
        {
    	  ...
    	  // here if there is an Alarm id that is not allocated
      	    Alarm[id].onTickHandler = NULL;
      	    Alarm[id].onTickArgHandler = onTickHandler;
    	    Alarm[id].Mode.isOneShot = isOneShot;
    	    Alarm[id].Mode.alarmType = alarmType;
    	    Alarm[id].value = value;
    	    Alarm[id].arg = arg;
