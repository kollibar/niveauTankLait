...
int8_t Timer::every(unsigned long period, void (*callback)(), int repeatCount)
{
	...
	_events[i].eventType = EVENT_EVERY | FCT_VIDE;
	_events[i].period = period;
	//_events[i].repeatCount = repeatCount;
	_events[i].callback = callback;
	// _events[i].callbackArg = NULL;
	// _events[i].callbackByte = NULL;
	// _events[i].arg = NULL;
	_events[i].lastEventTime = millis();
	//_events[i].count = 0;
	_events[i].count =  repeatCount;
	return i;
}
...
int8_t Timer::every(unsigned long period, void (*callback)(void*),void* arg, int repeatCount)
{
	int8_t i = findFreeEventIndex();
	if (i == -1) return -1;

	_events[i].eventType = EVENT_EVERY | FCT_POINTEUR;
	_events[i].period = period;
	//_events[i].repeatCount = repeatCount;
	_events[i].callback = (void (*)(void))callback;
	// _events[i].callback = NULL;
	// _events[i].callbackArg = callback;
	// _events[i].callbackByte = NULL;
	_events[i].arg = arg;
	_events[i].lastEventTime = millis();
	//_events[i].count = 0;
...
int8_t Timer::every(unsigned long period, void (*callback)(void*),void* arg)
{
	return every(period, callback, arg,-1); // - means forever
}

int8_t Timer::after(unsigned long period, void (*callback)(void*),void* arg)
{
	return every(period, callback, arg,1);
}

int8_t Timer::every(unsigned long period, void (*callback)(uint8_t),uint8_t val, int repeatCount)
{
	int8_t i = findFreeEventIndex();
	if (i == -1) return -1;

	_events[i].eventType = EVENT_EVERY | FCT_BYTE;
	_events[i].period = period;
	//_events[i].repeatCount = repeatCount;
	_events[i].callback = (void (*)(void))callback;
	// _events[i].callback = NULL;
	// _events[i].callbackArg = NULL;
	// _events[i].callbackByte = callback;
	// _events[i].arg = NULL;
	_events[i].val = val;
	_events[i].lastEventTime = millis();
	//_events[i].count = 0;
...
int8_t Timer::every(unsigned long period, void (*callback)(uint8_t),uint8_t val)
{
	return every(period, callback, val,-1); // - means forever
}

int8_t Timer::after(unsigned long period, void (*callback)(uint8_t),uint8_t val)
{
	return every(period, callback, val,1);
}

uint8_t Timer::getValue(int8_t id){
	//if( _events[id].callbackByte==NULL) return 255;
	if( (_events[id].eventType&B1100) != FCT_BYTE) return B11111111;
	return _events[id].val;
}
...
void Timer::update(void)
{
	...
	for (int8_t i = 0; i < MAX_NUMBER_OF_EVENTS; i++)
	{
		...
		{
			if( _events[i].update(now)) now=millis();
		}
	}
}
...
unsigned long Timer::next(unsigned long now){
